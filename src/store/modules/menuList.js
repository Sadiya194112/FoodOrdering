const menuList = {
  ...
  mutations: {
    ...
    assignTable(state, {tableId, orderId} ){
  ...
  actions: {
    reserveTable({commit, state}, {tableId}){
      const orderId = Date.now();
      const table = state.tableList.find((table) => table.id === tableId);
      if(table && !table.allocated){
        commit("assignTable", {tableId, orderId});
        return `Order ${orderId} has reserved table ${tableId}.`;
      }
      else{
